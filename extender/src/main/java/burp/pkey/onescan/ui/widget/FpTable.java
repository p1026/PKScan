package burp.pkey.onescan.ui.widget;
...
public class FpTable extends JTable {
    ...
    // 新增：存储当前表格每行数据对应的全局索引（FpManager.getList() 中的位置）
    private List<Integer> globalIndexes = new ArrayList<>();

    // 新增：设置数据时同步记录全局索引
    public void setDataWithGlobalIndexes(List<FpData> dataList, List<Integer> globalIndexes) {
        this.globalIndexes.clear();
        this.globalIndexes.addAll(globalIndexes);
        mTableModel.setData(dataList);
    }

    // 新增：通过本地行索引获取全局索引
    public int getGlobalIndex(int localRowIndex) {
        if (localRowIndex >= 0 && localRowIndex < globalIndexes.size()) {
            return globalIndexes.get(localRowIndex);
        }
        return -1;
    }

    public FpTableModel getTableModel() {
        return mTableModel;
    }
